...
API_SECRET = "YYY" if API_MOCK_ENABLED else conf.idex_api_secret_key
...
class IdexExchangeUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.market: IdexExchange = IdexExchange(API_KEY, API_SECRET, ["LINK-ETH", "ZRX-ETH"], True)
    ...
    def test_buy_and_sell(self):
        ...
        order_id = self.place_order(True, "LINK-ETH", amount, OrderType.LIMIT, bid_price, 10001, FixtureIdex.BUY_MARKET_ORDER,
                                    FixtureIdex.WS_AFTER_BUY_1, FixtureIdex.WS_AFTER_BUY_2)
    ...
    def test_cancel_all(self):
        ...
        buy_id = self.place_order(True, "LINK-ETH", quantized_amount, OrderType.LIMIT, quantize_bid_price, 10001,
                                  FixtureIdex.OPEN_BUY_ORDER, FixtureIdex.WS_AFTER_BUY_1,
                                  FixtureIdex.WS_AFTER_BUY_2)

        sell_id = self.place_order(False, "LINK-ETH", quantized_amount, OrderType.LIMIT, quantize_ask_price, 10002,
                                   FixtureIdex.OPEN_SELL_ORDER, FixtureIdex.WS_AFTER_SELL_1,
                                   FixtureIdex.WS_AFTER_SELL_2)

        self.run_parallel(asyncio.sleep(1))
        if API_MOCK_ENABLED:
            resp = self.fixture(FixtureIdex.CANCEL_ORDER, origClientOrderId=buy_id, side="BUY")
            self.web_app.update_response("delete", self.base_api_url, "/api/v3/order", resp,
                                         params={'origClientOrderId': buy_id})
            resp = self.fixture(FixtureIdex.CANCEL_ORDER, origClientOrderId=sell_id, side="SELL")
            self.web_app.update_response("delete", self.base_api_url, "/api/v3/order", resp,
    ...
    def test_order_price_precision(self):
        ...
        if API_MOCK_ENABLED:
            resp = self.order_response(FixtureIdex.ORDER_BUY_PRECISION, 1000001, "buy", "LINK-ETH")
            self.web_app.update_response("post", self.base_api_url, "/api/v3/order", resp)
        # Test bid order
        ...
        if API_MOCK_ENABLED:
            resp = FixtureIdex.ORDER_BUY_PRECISION_GET
        ...
        # Test ask order
        if API_MOCK_ENABLED:
            resp = self.order_response(FixtureIdex.ORDER_SELL_PRECISION, 1000002, "sell", "LINK-ETH")
            self.web_app.update_response("post", self.base_api_url, "/api/v3/order", resp)
        ...
        if API_MOCK_ENABLED:
            resp = FixtureIdex.ORDER_SELL_PRECISION_GET
        ...
        if API_MOCK_ENABLED:
            resp = self.fixture(FixtureIdex.CANCEL_ORDER, origClientOrderId=bid_order_id, side="BUY")
            self.web_app.update_response("delete", self.base_api_url, "/api/v3/order", resp,
                                         params={'origClientOrderId': bid_order_id})
            resp = self.fixture(FixtureIdex.CANCEL_ORDER, origClientOrderId=ask_order_id, side="SELL")
            self.web_app.update_response("delete", self.base_api_url, "/api/v3/order", resp,
    ...
    def test_orders_saving_and_restoration(self):
        ...
        try:
            ...
            if API_MOCK_ENABLED:
                resp = self.order_response(FixtureIdex.OPEN_BUY_ORDER, 1000001, "buy", "LINK-ETH")
                self.web_app.update_response("post", self.base_api_url, "/api/v3/order", resp)
            ...
            for event_tag in self.events:
                self.market.remove_listener(event_tag, self.market_logger)
            self.market: IdexExchange = IdexExchange(API_KEY, API_SECRET, ["LINK-ETH", "ZRX-ETH"], True)
            for event_tag in self.events:
                self.market.add_listener(event_tag, self.market_logger)
            recorder.stop()
            ...
            if API_MOCK_ENABLED:
                resp = self.fixture(FixtureIdex.CANCEL_ORDER, origClientOrderId=order_id, side="BUY")
                self.web_app.update_response("delete", self.base_api_url, "/api/v3/order", resp,
    ...
    def test_order_fill_record(self):
        ...
        try:
            ...
            order_id = self.place_order(True, "LINK-ETH", amount, OrderType.LIMIT, bid_price, 10001,
                                        FixtureIdex.BUY_LIMIT_ORDER, FixtureIdex.WS_AFTER_BUY_1,
                                        FixtureIdex.WS_AFTER_BUY_2)
            ...
            order_id = self.place_order(False, "LINK-ETH", amount, OrderType.LIMIT, ask_price, 10002,
                                        FixtureIdex.SELL_LIMIT_ORDER, FixtureIdex.WS_AFTER_SELL_1,
                                        FixtureIdex.WS_AFTER_SELL_2)
