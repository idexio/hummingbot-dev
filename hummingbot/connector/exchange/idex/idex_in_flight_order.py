...
class IdexInFlightOrder(InFlightOrderBase):
    ...
    def update_with_fill_update(self, fill_update: Dict[str, Any]) -> bool:
        ...
        """
        fill_id = fill_update.get("i", "fillId")
        if fill_id in self.fill_id_set:
            # fill already recorded
            return False
        self.fill_id_set.add(fill_id)
        self.executed_amount_base += Decimal(str(fill_update.get("q", "quantity")))
        self.fee_paid += Decimal(str(fill_update.get("f", "fee")))
        self.executed_amount_quote += (Decimal(str(fill_update.get("p", "price"))) *
                                       Decimal(str(fill_update.get("q", "quantity"))))
        if not self.fee_asset:
            self.fee_asset = fill_update.get("a", "feeAsset")
        return True
